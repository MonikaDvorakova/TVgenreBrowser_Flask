...
from CT_functions import (download_TVlisting_xml, max_date_calculation, find_shows_genre,
                         find_hyperlink, csfd_redirect, ask_csfd, date_reformat, set_week_day,
...
def index():
    ...
    if request.method == "GET":
        ...
        date_format_web = set_week_day(date)
        ...
        channels_format_web = change_channel_names(channels)
        if not searched_shows:
            ...
            return render_template("results.html", results=searched_shows_with_description, date=date_format_web, genre=genre, channels=channels_format_web)
